...
def load_kmers(kmer_file, expected_size=2304212, error_rate=0.001): #you can change the expected_size based on your kmer numbers
...
def contains_kmer(sequence, bloom_filter, kmer_size):
    # Iterate through the read sequence to check for presence of k-mers
...
def process_reads(bloom_filter, kmer_size, r1_file, r2_file, output_r1, output_r2, start, end):
    with gzip.open(r1_file, 'rt') as r1_handle, gzip.open(r2_file, 'rt') as r2_handle, \
        ...
        for i, (r1_record, r2_record) in enumerate(zip(r1_records, r2_records)):
            ...
            # Check if either paired read contains a k-mer
            if contains_kmer(str(r1_record.seq), bloom_filter, kmer_size) or contains_kmer(str(r2_record.seq), bloom_filter, kmer_size):
                # Write both reads to output if either contains a k-mer
                SeqIO.write(r1_record, out_r1, "fastq")
                SeqIO.write(r2_record, out_r2, "fastq")

# Parallel processing function
def process_in_parallel(bloom_filter, kmer_size, r1_file, r2_file, output_r1, output_r2, num_workers=4):
    # Count total number of reads in R1
    ...
    for i in range(num_workers):
        ...
        jobs.append((bloom_filter, kmer_size, r1_file, r2_file, output_r1, output_r2, start, end))
